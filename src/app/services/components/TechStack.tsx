      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {techCategories.map((cat, i) => (
            ...
            >
              <h4 className="text-[#0F2854] font-semibold text-sm tracking-wider uppercase mb-4">{cat.label}</h4>
              <div className="flex flex-wrap gap-2">
                {cat.items.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 rounded-lg border border-[#0F2854]/15 bg-[#0F2854]/5 text-[#0F2854] text-xs font-medium hover:border-[#0F2854]/35 transition-colors cursor-default"
                  >
                    {tech}
                  </span>
                ))}
