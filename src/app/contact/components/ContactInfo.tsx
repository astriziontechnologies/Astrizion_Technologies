...
export default function ContactInfo() {
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <div className="p-8 rounded-2xl border border-[#0F2854]/15 bg-white">
          ...
          {info.map((item) => {
            ...
            return (
              <div key={item.label} className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[#0F2854]/60 text-xs uppercase tracking-wider mb-0.5">{item.label}</p>
                  {item.href ? (
                    <a href={item.href} className="text-[#0F2854] text-sm hover:text-[#0F2854]/70 transition-colors break-all">
                      {item.value}
                    </a>
                  ) : (
      ...
      {/* Quick note */}
      <div className="p-6 rounded-2xl border border-[#0F2854]/15 bg-[#0F2854]/5">
        <div className="flex items-start gap-3">
          <MessageSquare size={18} className="text-[#0F2854] mt-0.5 shrink-0" />
          <div>
            <p className="text-[#0F2854] font-medium text-sm mb-2">Not sure where to start?</p>
            <p className="text-[#0F2854]/70 text-xs leading-relaxed">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {['LinkedIn', 'Twitter', 'GitHub', 'Instagram'].map((social) => (
            <a
              key={social}
              href="#"
              className="py-2 text-center text-xs text-[#0F2854]/60 border border-[#0F2854]/15 rounded-lg hover:bg-[#0F2854] hover:text-white hover:border-[#0F2854] transition-all"
            >
              {social}
            </a>
          ))}
